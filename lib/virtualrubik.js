...
function attachVirtualRubik(divOrCanvas, initCallback) {
 ...
 VirtualRubik.prototype.initScene = function() {
  ...
  for (var i=0;i<this.cube3d.stickerCount;i++) {
    ...
    if (i*2+1<stickerOffsets.length) {
    this.stickers[i].textureOffsetX=stickerOffsets[i*2];
    this.stickers[i].textureOffsetY=stickerOffsets[i*2+1];
    }
...
VirtualRubik.prototype.draw = function() {
  ...
  **/
  ...
  var scaleFactor =0.38*attr.scaleFactor;  // control the size of the rubik
