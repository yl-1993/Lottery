...
function includeVirtualRubik(baseURL) {
  ...
  */
function attachVirtualRubik(divOrCanvas, para, initCallback) {
  ...
  // get the console
   ...
   if (divOrCanvas==null) {
     ...
     var vr=new VirtualRubik(canvasElem, para);
  ...
  * object. 
  */
 VirtualRubik = function(canvas, para) {
   this.canvas=canvas;
   this.para = para;
 ...
 VirtualRubik.prototype.getNumType = function(ran_num) {
    return ran_num/2;
 }
 ...
 VirtualRubik.prototype.initScene = function() {
  ...
  this.xRot=attr.xRot;
  this.yRot=attr.yRot;
  this.camPos=new J3DIVector3(-0.5,-0.3,-6);
  ...
  var tmpStickerCount = this.cube3d.stickerCount; //54
  ...
  //console.log(tmpStickerCount);

  /*yl random number generation*/
  
  var k = 0, j = 0, ran_num = 0, ran_tmp = 0, ran_index = 0;
  ...
  for (var i = 0; i < 6; i++) {
    ...
    // shuffle card
    for(j = 0; j < 9; j++){
      ran_index = Math.floor(Math.random()*8);
      if(ran_index != j)
      {
        ran_tmp = cardArray[j];
        cardArray[j] = cardArray[ran_index];
        cardArray[ran_index] = ran_tmp;
      }
    }
    // set value according to the random card
    for (k = 9*i, j=0; k < 9*(i+1); k++, j++) { 
      this.stickers[k].textureOffsetX=stickerOffsets[2*cardArray[j]+0];
      this.stickers[k].textureOffsetY=stickerOffsets[2*cardArray[j]+1];
      // record the number
      this.squareNumber[k] = cardArray[j]+1;
    };
  };
  
  
/*  
  console.log(this.squareNumber[4]);
  console.log(this.squareNumber[22]);
  console.log(this.squareNumber[40]);
  console.log(this.squareNumber[31]);
  console.log(this.squareNumber[49]);
  console.log(this.squareNumber[13]);
  */
  /*
  console.log(this.squareNumber[1]);
  console.log(this.squareNumber[3]);
  console.log(this.squareNumber[5]);
  console.log(this.squareNumber[7]);
  console.log('10');
  console.log(this.squareNumber[10]);
  console.log(this.squareNumber[12]);
  console.log(this.squareNumber[14]);
  console.log(this.squareNumber[16]);
  console.log('19');
  console.log(this.squareNumber[19]);
  console.log(this.squareNumber[21]);
  console.log(this.squareNumber[23]);
  console.log(this.squareNumber[25]);
  console.log('28');
  console.log(this.squareNumber[28]);
  console.log(this.squareNumber[30]);
  console.log(this.squareNumber[32]);
  console.log(this.squareNumber[34]);
  console.log('37');
  console.log(this.squareNumber[37]);
  console.log(this.squareNumber[39]);
  console.log(this.squareNumber[41]);
  console.log(this.squareNumber[43]);
  console.log('46');
  console.log(this.squareNumber[46]);
  console.log(this.squareNumber[48]);
  console.log(this.squareNumber[50]);
  console.log(this.squareNumber[52]);
  console.log('---');
  */
...
VirtualRubik.prototype.reshape = function() {
    //console.log(this.para.pointedWidth);
    //console.log(this.para.pointedHeight);

    var gl=this.gl;
    var canvas = this.canvas;
    if (canvas.clientWidth == this.width && canvas.clientHeight == this.height)
        return;
    // cube size - resolution
    this.width = canvas.clientWidth;
    this.height = canvas.clientHeight;
    if(this.para.pointedWidth == 1024 && this.para.pointedHeight == 768){
      gl.viewport(-80, -30, this.width, this.height+300);
    }
    else{
      // the same with 1024*768
      gl.viewport(-80, -30, this.width, this.height+300);
    }   
...
VirtualRubik.prototype.draw = function() {
  ...
  **/
  wvMatrix.rotate(this.cube3d.attributes.xRot,2.1,-2.5,-0.2);
  ...
  var scaleFactor =0.34*attr.scaleFactor;  // control the size of the rubik -yl
...
//	this.world.add(this.cube3d); 
  ...
  for (var i=0;i<cube3d.sideCount;i++) {
      ...
      case 1: mvMatrix.rotate(90,0,0,1); break; //2
...
VirtualRubik.prototype.drawObject = function(obj, mvMatrix, color, phong, forceColorUpdate) {
  ...
  if (obj.textureScale!=null) {
    ...
    for (var i=0;i<textureArray.length;i+=2) {
      // by changing the textureScale to imitate the random number
      //var ran = Math.random()*6;
      var ran = 0;
      textureArray[i]=(obj.textureArray[i]+obj.textureOffsetX+ran)*obj.textureScale;
      textureArray[i+1]=(obj.textureArray[i+1]+obj.textureOffsetY+ran)*obj.textureScale;
    }
...
*   5: back
*/
VirtualRubik.prototype.PartToSideMap = [
   //40, 4, 49, 22, 31, 13
   13, 4, 22, 40, 31, 49
];


// yl: The fucking Notes caused me quite a long time...
VirtualRubik.prototype.RealToDeserveMap = [
  1, 0, 2, 4, 3, 5
];

VirtualRubik.prototype.getPartIndexFromSideIndex = function(location){
  return this.PartToSideMap[location];
}

VirtualRubik.prototype.PartToEdgeMap = [
  14, 1,
  3, 23,
  41, 7,
  46, 10,
...
VirtualRubik.prototype.getPartIndexFromEdgeIndex = function(location){
  return this.PartToEdgeMap[location];
}
...
VirtualRubik.prototype.drawResultList = function() {
  ...
  */
  //equal to above code
  resEdge[0] = this.cube.getEdgeAt(1);
  resEdge[1] = this.cube.getEdgeAt(10);
  this.explode();

  //var tmp = this.cube3d.getPartIndexForStickerIndex(22);
  //var tmp = this.cube.getEdgeOrientation(1);
  //console.log(this.RealToDeserveMap[resSide]);
  var partIndex = this.getPartIndexFromSideIndex(this.RealToDeserveMap[resSide]);
  //console.log(partIndex);
  var partNum = this.squareNumber[partIndex];
  //console.log(partNum);
  //console.log(resEdge[0]);
  //console.log(this.cube.getEdgeOrientation(resEdge[0]));
  //console.log(resEdge[1]);
  //console.log(this.cube.getEdgeOrientation(resEdge[1]));

  // uint (right edge)
  var index = resEdge[0];
  var resEdgeOri = this.cube.getEdgeOrientation(index);
  resEdgeOri = resEdgeOri==0?1:0;
  var edgePartIndex = this.getPartIndexFromEdgeIndex(2*index+resEdgeOri);
  //console.log(edgePartIndex);
  //console.log(this.squareNumber[edgePartIndex]);
  var uintNum = this.squareNumber[edgePartIndex];

  // hundred (left edge)
  index = resEdge[1];
  resEdgeOri = this.cube.getEdgeOrientation(index);
  resEdgeOri = resEdgeOri==0?1:0;
  edgePartIndex = this.getPartIndexFromEdgeIndex(2*index+resEdgeOri);
  //console.log(edgePartIndex);
  //console.log(this.squareNumber[edgePartIndex]);
  var hundredNum = this.squareNumber[edgePartIndex];
