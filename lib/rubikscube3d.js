...
RubiksCube3D.prototype.updateExplosionFactor=function(resSide, resEdge, factor) {
  if (factor == null) {
    ...
    //console.log(resEdge);
        ...
        // yl: if you want to explode the conrner, pls active the below code
        /*
        for (var i = 0; i < this.cornerCount; i++) {
        ...
        for (var i = 0; i < this.edgeCount; i++) {
            //only expolde the front and middle edge
            if(i != resEdge[0] && i != resEdge[1])
              continue;
        ...
        for (var i = 0; i < this.sideCount; i++) {
            //only expolde the front side
            if(i!=resSide) continue;
